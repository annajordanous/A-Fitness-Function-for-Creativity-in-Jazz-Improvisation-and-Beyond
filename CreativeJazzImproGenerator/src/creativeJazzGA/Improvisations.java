	...
	*//**
	...
	void ratePopulationWithRitchie()  {
		...
		if (populationCapacity >= 2)  {   // if only one impro, can only rate one - otherwise rate two impros
			...
			rateChosenSampleWithRitchie(index2);
		}
		propogateRatingsWithRitchie(index1, index2);
	}

	/**
	 * @param index
	 */
	private void rateChosenSampleWithRitchie(int index) {
		String filename;
		filename = makeFilename(index);
		// View.notate(population[i]);
		playMidiFile(filename, population[index]);	
		ratePopulationMemberWithRitchie(index); 
		stopPlayback();
	}
	 ...
	 */
	private void propogateRatingsWithRitchie(int index1, int index2) {
	 ...
	 */
	private void ratePopulationMemberWithRitchie(int index) {
